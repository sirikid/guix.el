...
These are \\+`fake' arguments that are not presented in \\+`guix'
shell commands.")
...
(defvar guix-command-post-processors
  '(("environment"
    ...
    ("system"
     guix-command-post-process-rest-single))
  "Guix commands and their post-processing functions.
Alist of guix commands and functions for post-processing a list
of arguments returned from popup interface.  Each function is
called on the returned arguments in turn.")
...
(cl-defun guix-command-post-process-matching-args
    (args regexp &key group split?)
  "Transform matching arguments.
Modify arguments from ARGS matching REGEXP by moving them to the
end of ARGS list.  You can use GROUP instead of the whole
argument.  If SPLIT? is non-nil, split matching arguments into
...
(defun guix-command-post-process-rest-multiple (args)
  "Split arguments after \\+`--' and remove it.
Modify ARGS by splitting `-- ARG ...' into multiple subarguments
and moving them to the end of ARGS list.  Remove '-- ' string."
  (guix-command-post-process-matching-args
   args guix-command-rest-arg-regexp
   :group 1
   :split? t))
...
(defun guix-command-post-process-rest-multiple-leave (args)
  "Split arguments after \\+`--'.
Modify ARGS by splitting `-- ARG ...' into multiple subarguments
and moving them to the end of ARGS list.  Leave \\+`--' string as
a separate argument."
  (guix-command-post-process-matching-args
   args guix-command-rest-arg-regexp
   :split? t))
...
(defun guix-run-view-build-log (args)
  "Run `guix ARGS ...' build command and show the log file(s)."
