...
Should be a symbol \\+`package' or \\+`output' (if \\+`output',
display each output on a separate line; if \\+`package', display
...
(defun guix-package-entry-installed-outputs (entry)
  ...
  (mapcar (lambda (it)
            (bui-entry-non-void-value it 'output))
...
ENTRIES is a list of package entries to get info about packages."
  (remq
   ...
   (mapcar (cl-function
            (lambda ((id . outputs))
              (let ((it (bui-entry-by-id entries id)))
                (when it
...
\\+`Unknown' means there are no packages with this name among
Guix packages.  Perhaps you installed this package in the past,
and it was renamed later."
  :group 'guix-package-info-faces)

(defface guix-package-info-obsolete
  '((t :inherit font-lock-warning-face))
  "Face used for obsolete packages.
\\+`Obsolete' means there is a Guix package with this name but
with a newer version (probably it's time to update)."
  :group 'guix-package-info-faces)
      ...
      (cl-case (bui-entry-non-void-value entry 'known-status)
        (known
         (when (bui-entry-non-void-value entry 'non-unique)
           ...
           (bui-newline))
         (let ((it (bui-entry-non-void-value entry 'superseded)))
           (when it
...
The current profile is the one used by the current buffer."
  (let ((it (guix-ui-current-profile)))
    (when it
...
FUN should take action-type as argument."
  ...
  (let ((actions (remq nil (mapcar fun '(install delete upgrade)))))
    (if actions
...
(defun guix-output-list-describe (&rest ids)
  ...
  (let ((pids (mapcar (lambda (it)
                        (car (guix-package-id-and-output-by-output-id it)))
                      ids)))
    (bui-get-display-entries
     'guix-package 'info
...
Make sure FILE has a proper \\+`operating-system' declaration.
You may check it, for example, by running the following shell
command:
...
services (like \\+`guix' or \\+`shepherd').  To see all the
packages installed in a system profile, use
...
TYPE should be a symbol \\+`all' or \\+`direct'.  Interactively,
prompt for it.
